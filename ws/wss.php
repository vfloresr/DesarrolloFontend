...
function set_editar_cumple_auto()
{
	$configuraciones = new Configuraciones;
	$url = $configuraciones->getUrlPrototipo();
    $url_plantilla = (isset($_REQUEST['html'])) ? $_REQUEST['html']: '';
	if($url_plantilla != ''){
		$templatefile = 'http://crm.cochadigital.com/templates/'.$url_plantilla;
	...
	$contactos = $_REQUEST['contact_id'];
    $array = explode(',',$contactos);

	 $parametros = array(
   'session' => $_SESSION['session_id'],
   'usuario_asig' => $_SESSION['user_id'],
    'delete' =>$_REQUEST['estado'],
	'html'=>  $page,
	'asunto' => 'Saludo de Cumpleaños',
	'comentarios' => (isset($_REQUEST['saludos_ejecutivo'])) ? $_REQUEST['saludos_ejecutivo']: ' ', 
	'campo' => $array
 ...
 //var_dump($parametros);	
   ...
   $result = $configuraciones->call("set_editar_cumple_auto", $parametros, $url);
...
function set_enviar_email_web()
{
	...
	$session = (isset($_SESSION['session_id'])) ? $_SESSION['session_id']  : $_REQUEST['id_session'];
	$id_user = (isset($_SESSION['user_id'])) ? $_SESSION['user_id']  :  $_REQUEST['id_user'];
	$correo = (isset($_REQUEST['email'])) ? $_REQUEST['email']:'';
    $asunto = (isset($_REQUEST['asunto'])) ? $_REQUEST['asunto']  :  '';
	$texto = (isset($_REQUEST['texto_hide'])) ? $_REQUEST['texto_hide']  :  '';
	$contacto_id = (isset($_REQUEST['contacto_id'])) ? $_REQUEST['contacto_id']:'';
	//echo "contacto:".$contacto_id."<br>";
    //echo "Usuario Asignado:".$id_user."\n";	
	//die();
		
	$email_cc = (isset($_REQUEST['email_cc'])) ? $_REQUEST['email_cc']  :  '';
	$ruta_upload = (isset($_REQUEST['t_accion'])) ? $_REQUEST['t_accion']  :  '';
	$from = (isset($_SESSION['email'])) ? $_SESSION['email']  :  $_REQUEST['from'];
	$nombre_user = (isset($_SESSION['nombre'])) ? $_SESSION['nombre']  :  $_REQUEST['nombre_user'];
	$apellido_user = (isset($_SESSION['apellido'])) ? $_SESSION['apellido']  :  $_REQUEST['apellido_user'];
	...
	$parametros = array(
		...
		't_accion' => $t_accion,
		'from' => $from,
		'nombre_user' => $nombre_user.' '.$apellido_user,
		'procedencia' => $procedencia,
		'procedencia_id' => $procedencia_id,
		'pdf' => $pdf,
		'estado'=>$estado
	...
	// $parametros = array(
		// 'session' => 'armando',
		// 'user_id' => 'bdbc32fa-200d-2aed-4611-5660b86e05dc',
		// 'email' => $correo,
		// 'asunto' => $asunto,
		// 'texto' => $texto,
		// 'email_cc' => $email_cc,
...
function set_editar_oportunidad_web()
{
	...
	$parametros = array(
		...
		'oportunidad_id' => $_REQUEST['oportunidad_id'],
		'campos_opor' => array(
		    'tipo_cierre_c'     => $_REQUEST['valor_select'],
			'numero_negocio_c' => $_REQUEST['numero_negocio_c'],
			'amount' => 		  $_REQUEST['amount'],
...
//*****************************CROSSELLING****************************************************************//
function set_editar_cambio_estado_cross_selling (){
 	$configuraciones = new Configuraciones;
	$url = $configuraciones->getUrlPrototipo();
	$parametros = array(
		'session' => $_SESSION['session_id'],
	...
	$result = $configuraciones->call("set_editar_cambio_estado_cross_selling", $parametros, $url);
...
function set_editar_cambio_estado_recompra (){
 	$configuraciones = new Configuraciones;
	$url = $configuraciones->getUrlPrototipo();
	$parametros = array(
		'session' => $_SESSION['session_id'],
	...
	$result = $configuraciones->call("set_editar_cambio_estado_recompra", $parametros, $url);
...
function set_editar_cambio_estado_fugados (){
 	$configuraciones = new Configuraciones;
	$url = $configuraciones->getUrlPrototipo();
	$parametros = array(
		'session' => $_SESSION['session_id'],
	...
	$result = $configuraciones->call("set_editar_cambio_estado_fugados", $parametros, $url);
