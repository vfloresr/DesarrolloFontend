jQuery(document).ready(function($){ 
	...
	$('#seguimiento_2_oportunidades_web_success').on('click', function(){
		var descripcion = $('input[name=solicitudes_web_comentarios]').val().trim()+" *|*|*|* "+ FuncionesGenerales.getFechaHoraActual() +": "+$('#comentarios_seguimiento_2_oportunidades_web').val().trim();
		var edo =  $('input[name=solicitud_edo]').val();
		
		var $btn = $(this);
		$btn.button('loading');
		var data = { 
			  oportunidad_id: ($('input[name=solicitudes_web_oportunidad_id]').val() == '' ? edo: $('input[name=solicitudes_web_oportunidad_id]').val() ),
			
			 //oportunidad_id:'36d51692-c24f-8418-956c-5469cd7bcb15',
			  numero_negocio_c: '',
			  description: descripcion.trim(),
			  amount: '0',
			  motivo_c: '',
			  sale_stage: Configuraciones.getConfiguraciones().tag_sale_stage_seguimiento_2,//negociacion
		};
		...
		$.post(Configuraciones.getConfiguraciones().url_set_editar_oportunidad_web, data).done(function(){
			...
			else
			{
				...
				$('#seguimiento_2').modal('toggle');
				$btn.button('reset');	
				//$('input[name=solicitud_edo]').val("");
			
			}
		});
	});

	$('#seguimiento_2_oportunidades_web_cancel').on('click', function(){
		$('#seguimiento_2').modal('toggle');
	});
	
	$('#seguimiento_2').on('hide.bs.modal', function (e){
		$('#comentarios_seguimiento_2_oportunidades_web').val('');
...
function modal_seguimiento_dos_oportunidades_web(el)
{
	if( typeof $("#solicitudes_web_oportunidades").data('bs.modal')  != 'undefined' && $("#solicitudes_web_oportunidades").data('bs.modal').isShown)
