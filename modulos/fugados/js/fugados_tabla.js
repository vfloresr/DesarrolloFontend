					  ...
					  ajax: {
						...
						'dataSrc': function ( json ) {
							...
							for(i = 0; i < json.length; i++){
								...
								json[i].id_negocio = "<a href='#' onclick='abrirDetalleNegocioModal(this);' data-oportunidad-id='"+json[i].id_oportunidad.trim()+"' data-json='"+ json_aux +"'>" + json[i].id_negocio + "</a>";
								//json[i].buttons = '';
								json[i].buttons ="<button type='button' class='btn btn-rounded btn-primary btn-block' onclick='abrirFugados(this);' data-oportunidad-id='"+json[i].id_oportunidad.trim()+"' data-json='"+ json_aux+"' title='Abrir Fugados'>Abrir</button>";						
