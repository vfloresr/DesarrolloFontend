				...
				if(!$.fn.dataTable.isDataTable('#abierto_table_recompra'))
				{
						   ...
						   ajax: {
							...
							'dataSrc': function ( json ) {
								...
								for(i = 0; i < json.length; i++){
									...
									json[i].destino = '<div style="padding:10px;color:#696969;"><strong style="color:#696969;">' +  json[i].destino  + '</strong>  <span class="label label-default">' + json[i].dias + ' días</span> </br>' + ((json[i].ruta!==null)?json[i].ruta:'Sin Ruta') + ' </div>';
					...
					$.fn.dataTableExt.oApi.fnActualizarRegistroTablaRecompraNuevas = function ()
					{
							...
							$.get(Configuraciones.getConfiguraciones().url_busca_oportunidad, data).done(function(response)
							{
								...
								var destino = '<div style="padding:10px;color:#696969;"><strong style="color:#696969;">' +  json.destino  + '</strong>  <span class="label label-default">' + json.dias + ' días</span> </br>' + ((json.ruta!==null)?json.ruta:'Sin Ruta') + ' </div>';
				  ...
				  RecompraTabla.cerrado_recompra = $('#cerrados_table_recompra').dataTable({
					  ajax: {
						...
						'dataSrc': function ( json ) {
							...
							for(i = 0; i < json.length; i++){
								...
								//json[i].buttons = '';
								//json[i].buttons = "<a href='#' onclick='abrirRecompra(this);' data-oportunidad-id='"+json[i].id_oportunidad.trim()+"' data-json='"+ json_aux+"'><span class='glyphicons glyphicons-unlock' title='Abrir Recompra' style='color:#696969;'><p style='color:#696969;font-size:12px;font-family: 'Arial', Times New Roman, Georgia, Serif;font-weight:bold;margin-top:5px;' >Abrir</p></span></a>";
								json[i].buttons ="<button type='button' class='btn btn-rounded btn-primary btn-block' onclick='abrirRecompra(this);' data-oportunidad-id='"+json[i].id_oportunidad.trim()+"' data-json='"+ json_aux+"' title='Abrir Recompra'>Abrir</button>";						
								json[i].destino = '<div style="padding:10px;color:#696969;"><strong style="color:#696969;">' +  json[i].destino  + '</strong>  <span class="label label-default">' + json[i].dias + ' días</span> </br>' + ((json[i].ruta!==null)?json[i].ruta:'Sin Ruta') + ' </div>';
								json[i].nombre_comprador = '<div style="padding:10px;color:#696969;"><strong style="color:#696969;">'+json[i].nombre_comprador+'</strong></br> Rut: '+json[i].rut_comprador+'</div>';
								json[i].id_negocio = "<a href='#' onclick='abrirDetalleNegocioModal(this);' data-oportunidad-id='"+json[i].id_oportunidad.trim()+"' data-json='"+ json_aux +"'>" + json[i].id_negocio + "</a>";
				  	...
				  	$.fn.dataTableExt.oApi.fnActualizarRegistroTablaRecompraCerrados = function ()
					{
							...
							$.get(Configuraciones.getConfiguraciones().url_busca_oportunidad, data).done(function(response)
							{
								...
								var buttons = '';
								var id_negocio = "<a href='#' onclick='abrirDetalleNegocioModal(this);' data-oportunidad-id='"+json.id_oportunidad.trim()+"' data-json='"+ json_aux +"'>" + json.id_negocio + "</a>";
								var destino = '<div style="padding:10px;color:#696969;"><strong style="style="color:#696969;">' +  json.destino  + '</strong>  <span class="label label-default">' + json.dias + ' días</span> </br>' + ((json.ruta!==null)?json.ruta:'Sin Ruta') + ' </div>';
								var nombre_comprador = '<div style="padding:10px;color:#696969;"><strong style="style="color:#696969;">'+json.nombre_comprador+'</strong></br> Rut: '+json.rut_comprador+'</div>';
								var monto_venta = json.monto_venta;
