jQuery(document).ready(function() {
	...
	//pop over cross-selling
  			
			$('.cross_menu').webuiPopover({
			  content: function(){
			  return $('#popover_cross').html();
			},
			  width:'300px',//can be set with  number 
		...
		//pop over Recompra
			$('.recompra_menu').webuiPopover({
					content: function(){
				return $('#popover_recompra').html();
			},
				width:'300px',//can be set with  number
				height:'auto',//can be set with  number
				trigger:'hover',
			...
			//pop over fugados
			$('.fugados_menu').webuiPopover({
					content: function(){
				return $('#popover_fugados').html();
			},
				width:'300px',//can be set with  number
				height:'auto',//can be set with  number
				trigger:'hover',
		   	...
		   	//pop over proximos viajes
			$('.proxviajes_menu').webuiPopover({
					content: function(){
				return $('#popover_proxviajes').html();
			},
				width:'300px',//can be set with  number
				height:'auto',//can be set with  number
				trigger:'hover',
			...
			//pop over retorno
			$('.retorno_menu').webuiPopover({
					content: function(){
				return $('#popover_retorno').html();
			},
				width:'300px',//can be set with  number
				height:'auto',//can be set with  number
				trigger:'hover',
			...
			//pop over cumpleaños
			$('.cumpleanios_menu').webuiPopover({
					content: function(){
				return $('#popover_cumpleanio').html();
			},
				width:'300px',//can be set with  number
				height:'auto',//can be set with  number
				trigger:'hover',
	...
	$('#email_solicitud_web').on('shown.bs.modal', function (e) {
		...
		$('#detalles_pasajero').webuiPopover({
			content: function(){
	...
	$("select[name|='mail_modal_solicitud_web_destinos_value']").change(function() {
		...
		$.get(Configuraciones.getConfiguraciones().url_cocha_csm_mobile_api + "/programas?destino="+destino_id).done(function( programas ) {
			 ...
			 for (var i in programas) { 
				$("select[name|='mail_modal_solicitud_web_programas_value']").append('<option data-days="' + programas[i].days + '" data-title="' + programas[i].title + '" data-nights="' + (programas[i].days-1) + '" data-subtitle="' + programas[i].subtitle + '" value="'+programas[i].id+'">' + programas[i].days + ' D\u00ECas, ' + programas[i].title + ', ' + programas[i].subtitle + '</option>');
			 }
		});
	});
	...
	$("select[name|='mail_modal_solicitud_web_programas_value']").change(function() {
		 ...
		 $('#title').html('<h3><ins><strong>Detalle Programa</strong></ins></h3>' + $(this).find(':selected').data('title') + '<br/>'+ $(this).find(':selected').data('subtitle') + '<br/>' + $(this).find(':selected').data('days') + 'D\u00ECas ' + $(this).find(':selected').data('nights') +' Noches </br></br>');
		 $('input[name=mail_modal_solicitud_web_asunto_value]').val('Cotizaci\u00f3n '+ $(this).find(':selected').data('title') +' '+ $(this).find(':selected').data('days') +' D\u00ECas / '+ $(this).find(':selected').data('nights') +' Noches');
	    	FuncionesGenerales.getCKEditorPlantillaCotizacion().document.getById('programas').setHtml('Cotizaci\u00f3n '+ $(this).find(':selected').data('title') +' '+ $(this).find(':selected').data('days') +' D\u00ECas  / '+ $(this).find(':selected').data('nights') +' Noches');
     });
	...
	$("select[name|='mail_modal_solicitud_web_documentacion_value']").change(function() {
		if( $(this).val() != -999)
			...
			FuncionesGenerales.getCKEditorPlantillaCotizacion().document.getById('cgeneral').setHtml("<br><h3><ins><strong>Documentaci&oacute;n</strong></ins></h3>"+ $(this).val());
		else
			// $('#cgeneral').html("");
	...
	$('input[name=mail_modal_solicitud_web_incluye_value]').change(function() {
		...
		if($(this).is(":checked")) 
		{
			$.get(Configuraciones.getConfiguraciones().url_cocha_csm_mobile_api + "/programas?id="+programa_id).done(function( programa ) {
				//console.log(programa[0].includeDescriptions.aereo[0]);
				var pasa = 0;
				if(!$.isEmptyObject(programa[0].includeDescriptions))
				{	
					for (var key in programa[0].includeDescriptions) {
					   var obj = programa[0].includeDescriptions[key];
					   for (var prop in obj) {
						  if((obj[0] == '') || (typeof obj[0] == 'undefined')){
							  pasa = 1;
						  }
					   }
					} 
