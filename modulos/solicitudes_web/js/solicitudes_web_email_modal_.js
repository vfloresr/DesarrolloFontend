jQuery(document).ready(function() {
	...
	$('#email_solicitud_web').on('shown.bs.modal', function (e) {
		...
		$.get('http://cms.cocha.com/mobile-api/destinos').done(function( destinos ) {
			 $("select[name|='mail_modal_solicitud_web_destinos_value']").html('');
	...
	$("select[name|='mail_modal_solicitud_web_destinos_value']").change(function() {
		...
		$.get("http://cms.cocha.com/mobile-api/programas?destino="+destino_id).done(function( programas ) {
			 $("select[name|='mail_modal_solicitud_web_programas_value']").html('');
	...
	$("select[name|='mail_modal_solicitud_web_documentacion_value']").change(function() {
		if( $(this).val() != -999)
			...
			FuncionesGenerales.getCKEditorPlantillaCotizacion().document.getById('cgeneral').setHtml("<br><h3><ins><strong>Documentaci\u00f3ns</strong></ins></h3>"+ $(this).val());
		else 
			// $('#cgeneral').html("");
	...
	$('input[name=mail_modal_solicitud_web_incluye_value]').change(function() {
		...
		if($(this).is(":checked")) 
		{
			$.get("http://cms.cocha.com/mobile-api/programas?id="+programa_id).done(function( programa ) {
				// $('#incluye').html('').html('Incluye: </br><ul>' + programa[0].description + '</ul>'); 
				FuncionesGenerales.getCKEditorPlantillaCotizacion().document.getById('incluye').setHtml('Incluye: </br><ul>' + programa[0].description + '</ul>');
			});
		}
		else
		{
	...
	$('input[name=mail_modal_solicitud_web_imagen_value]').change(function() {
		...
		{
			$.get("http://cms.cocha.com/mobile-api/programas?id="+programa_id).done(function( programa ) {
				var imagen = "http://cms.cocha.com/" + programa[0].imgUrl;
